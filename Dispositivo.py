...
async def _http_post_json(url, payload, headers=None, timeout_ms=3000, read_full=False, max_full=4096):
    import json, time
    ...
    try:
        ...
        if headers:
            for k, v in headers.items():
                lk = k.lower()
                if lk not in ("host","content-type","content-length","connection"):
                    req += "{}: {}\r\n".format(k, v)
        req += "\r\n" + body
        ...
        # ---- BODY
        if read_full:
            # acumula hasta cl, pero con un tope para no matar la RAM
            body_bytes = rest
            if cl is not None and cl <= max_full:
                ...
                # si no hay CL o es muy grande, solo toma un pedazo seguro
                while len(body_bytes) < max_full:
                    chunk = await reader.read(max_full - len(body_bytes))
                    if not chunk:
                        break
                    body_bytes += chunk
                # descarta el resto
            ...
            # modo seguro: preview pequeño + descartar
            preview = rest[:512] if rest else b""
            try:
                while True:
                    if not await reader.read(256):
                        break
...
async def ping_api(name="ESP32-Atleta"):
    global ATHLETE_ID, ATHLETE_HEIGHT, ATHLETE_NAME
    url = API_BASE + "/api/devices/ping"
    status, txt = await _http_post_json(url, {"name": name}, read_full=True)  # <-- lee completo
    print("Ping:", status, txt)
    if 200 <= status < 300:
        try:
            data = json.loads(txt or "{}")
            if data.get("assigned_athlete_id") is not None:
                ATHLETE_ID = int(data["assigned_athlete_id"])
                print("Asignado a atleta:", ATHLETE_ID)
            ATHLETE_HEIGHT = data.get("athlete_height")
            ATHLETE_NAME   = data.get("athlete_name")
            if ATHLETE_NAME:   print("Nombre atleta:", ATHLETE_NAME)
            if ATHLETE_HEIGHT: print("Altura atleta (cm):", ATHLETE_HEIGHT)
        except Exception as e:
            print("JSON error:", e)
        return True
    return False
...
async def enviar_metricas(bpm, repeticiones):
    ...
    payload = {"bpm": bpm, "repeticiones": repeticiones, "exercise": (titulo or "N/A")}
    status, txt = await _http_post_json(url, payload, read_full=False)  # <-- modo seguro
    print("Metrics:", status, txt)  # txt será un “preview” corto
